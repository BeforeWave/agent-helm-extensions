...
const agentHelmReleaseVersion = option('--agent-helm-release-version') ?? agentHelmVersion
...
if (!agentHelmReleaseVersion || !/^\d+\.\d+\.\d+(?:-dev)?$/.test(agentHelmReleaseVersion)) throw new Error('--agent-helm-release-version must be x.y.z or x.y.z-dev')
if (agentHelmReleaseVersion.replace(/-dev$/, '') !== agentHelmVersion) throw new Error('Agent Helm release version must target the pinned Agent Helm product version')
...
echo Agent Helm ${agentHelmVersion} - Chrome Extension ${extensionId} - Release v${agentHelmReleaseVersion}\r
echo.\r
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $source=(Invoke-WebRequest -UseBasicParsing -Uri '${installUrl}').Content; if ([string]::IsNullOrWhiteSpace($source)) { throw 'Agent Helm install.ps1 is empty' }; & ([scriptblock]::Create($source)) -Version '${agentHelmReleaseVersion}' -ChromeExtensionId '${extensionId}'; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }"\r
